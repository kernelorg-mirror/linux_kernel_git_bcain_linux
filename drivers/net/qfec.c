...
static int qfec_rbd_init(struct net_device *dev, struct buf_desc *p_bd)
{
	...
	skb   = dev_alloc_skb(ETH_BUF_SIZE + NET_IP_ALIGN);
	if (!skb)
		goto err;
	...
	unsigned int dplx)
{
	...
	QFEC_LOG(QFEC_LOG_DBG, "%s: %d spd, %d dplx\n", __func__, spd, dplx);
...
static int qfec_mdio_show(char *buf, char **start, off_t offset,
        int count, int *eof, void *data)
{
        struct qfec_priv        *priv = netdev_priv((struct net_device *)data);
        int                      n = 32;
        ...
        for (n=0; n< 32; n++) {
                l += sprintf(&buf[l], "   Reg%d  %8x\n", n,
                        qfec_mdio_read((struct net_device *)data, priv->phy_id,n));
        }
...
static void qfec_get_an(struct net_device *dev, uint32_t *spd, uint32_t *dplx)
{
    ...
    if ( rgmii_mode ) {
    	/* read advertised phy */
	adv = qfec_mdio_read(dev, priv->phy_id, MII_ADVERTISE);
        anExp = qfec_mdio_read (dev, priv->phy_id, MII_EXPANSION);
	mastCtrl = qfec_mdio_read(dev, priv->phy_id, MII_CTRL1000);

	anPartner = qfec_mdio_read(dev, priv->phy_id, MII_LPA);
	mastStat = qfec_mdio_read(dev, priv->phy_id, MII_STAT1000);


    	/* read PHY status register and check if auto-neg complete */
	status = qfec_mdio_read(dev, priv->phy_id, MII_BMSR);

    	/* AND advertised and partner capabilities to determine speed/duplex */
    	adv &= anPartner;
		...
		*dplx = 0;
    	...
    	{
		*spd  = spd_100;
		*dplx = status & BMSR_100FULL ? 1 : 0;
		QFEC_LOG(QFEC_LOG_DBG, "%s: 100M FD HD\n", __func__);
    	}

    	else if (adv & (ADVERTISE_10HALF | ADVERTISE_10FULL) )
    	{
		*spd  = spd_10;
		*dplx = status & BMSR_10FULL ? 1 : 0;
		QFEC_LOG(QFEC_LOG_DBG, "%s: 10M FD HD\n", __func__);
    	}
	...
	if (status & ADVERTISE_PAUSE_CAP)  {
		...
		else if (adv & ADVERTISE_PAUSE_CAP)
...
static void qfec_rx_int(struct net_device *dev)
{
	...
	//  Could probaby be a little smarter about this
	qfec_reg_write(priv, RX_POLL_DEM_REG, 1);
}

/*  NAPI stuff  */
#ifdef QFEC_NAPI

static int qfec_napi_poll(struct napi_struct *napi, int budget)
{
	...
	while (!(desc_status & BUF_OWN) && (!qfec_ring_full(p_ring)))  {
		...
		if (unlikely(desc_status & BUF_RX_ES)) {
			...
			if (NET_RX_DROP == netif_receive_skb(skb))  {
				priv->stats.rx_dropped++;
				CNTR_INC(priv, rx_dropped);
			}
			CNTR_INC(priv, netif_rx_cntr);
	...
	if (rxPkts < budget) {
		napi_complete(napi);
		//  re-enable interrupt -- stale packet problem?
		qfec_reg_write(netdev_priv(napi->dev), INTRP_EN_REG, QFEC_INTRP_SETUP);
	}
...
static irqreturn_t qfec_int(int irq, void *dev_id)
{
...
#ifdef QFEC_NAPI
		//  schedule the napi poll instead of processing packets
		napi_schedule(&((struct qfec_priv *) netdev_priv(dev))->napi);
		//  disable the RX interrupt
		qfec_reg_write(priv, INTRP_EN_REG, QFEC_INTRP_SETUP & ~INTRP_EN_REG_RIE);
#else
		qfec_rx_int(dev);
#endif
	}
	...
	if (status & (STATUS_REG_GPI | STATUS_REG_GMI | STATUS_REG_GLI))  {
		...
		qfec_reg_read(priv, SGMII_RGMII_SMII_STATUS_REGISTER);
	}
...
static int qfec_open(struct net_device *dev)
{
	...
	if (mii_check_gmii_support(&priv->mii)) {
...
#ifdef QFEC_NAPI
	napi_enable(&((struct qfec_priv *)netdev_priv(dev))->napi);
#endif
	netif_start_queue(dev);

#ifdef CONFIG_SMP
	//  omg hax
	smp_call_function(smp_vm_unmask_irq, (void *) dev->irq, 0);
#endif

	QFEC_LOG(QFEC_LOG_DBG, "%s: %08x link, %08x carrier\n", __func__,
		mii_link_ok(&priv->mii), netif_carrier_ok(priv->net_dev));
	qfec_phy_monitor((unsigned long)dev);
...
#ifdef QFEC_NAPI
	napi_disable(&((struct qfec_priv *)netdev_priv(dev))->napi);
#endif
...
static int qfec_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	qfec_reg_write(priv, TX_POLL_DEM_REG, 1);      /* poll */
        ...
        if (rgmii_mode) {
	    qfec_reg_read(priv, SGMII_RGMII_SMII_STATUS_REGISTER); /* Clear RGMII */
	    qfec_reg_write(priv,  INTRP_MASK_REG, 1);  /* Disable RGMII int */
	}
done:
	spin_unlock_irqrestore(&priv->xmit_lock, flags);
...
#ifdef QFEC_NAPI
        netif_napi_add(dev, &((struct qfec_priv *) netdev_priv(dev))->napi,
                       qfec_napi_poll, 32);
#endif
