...
static inline void msm_gpio_set_irq_handler(struct device *dev)
{
	...
	if (!dev->of_node) {
		for (i = 0; i < msm_gpio.gpio_chip.ngpio; ++i) {
			...
			printk("%s mapping gpio hwirq %d to %d\n", __func__, i, irq);
...
static int msm_gpio_probe(struct platform_device *pdev)
{
	...
	printk("%s tlmm_msm_summary_irq %d\n", __func__, tlmm_msm_summary_irq);
...
//  yes, this is a no-no; we need to pass this domain to the platform
//  so it can get the mapping for the ethernet IRQ.  devtree I think
//  would fix this.  need 8960 to use moar devtree, plzkthx.
extern struct irq_domain *gpio_irqdomain;
#endif
...
#ifdef CONFIG_HEXAGON_MSM8960_FLUID
	//  I need to stop with the greasy hacks
	gpio_irqdomain = msm_gpio.domain;
	for (i = 0; i < ngpio; ++i) {
		irq = irq_create_mapping(msm_gpio.domain, i);
		irq_set_lockdep_class(irq, &msm_gpio_lock_class);
		irq_set_chip_and_handler(irq, &msm_gpio_irq_chip,
					 handle_level_irq);
		//set_irq_flags(irq, IRQF_VALID);
