...
#define INTR_TARGET_PROC_SENSORS	(1 << INTR_TARGET_PROC_BIT)
#define INTR_TARGET_PROC_LPA_DSP	(2 << INTR_TARGET_PROC_BIT)
#define INTR_TARGET_PROC_APPS		(4 << INTR_TARGET_PROC_BIT)
#define INTR_TARGET_PROC_MSS		(5 << INTR_TARGET_PROC_BIT)
...
void __msm_gpio_set_intr_cfg_type(unsigned gpio, unsigned type)
{
	...
	//  This really needs to just go into the devtree
#ifdef CONFIG_HEXAGON_MSS
	cfg = INTR_RAW_STATUS_EN | INTR_TARGET_PROC_MSS;
#else
	cfg = INTR_RAW_STATUS_EN | INTR_TARGET_PROC_LPA_DSP;
#endif
