...
/* Cap on set_next_event() retries when the deadline has already elapsed. */
#define MAX_DELTA_RETRIES	100
...
static int set_next_event(unsigned long delta, struct clock_event_device *evt)
{
	 ...
	 * called rather than the time it completes.  Retry a bounded number
	 * of times; if the deadline keeps slipping past, report -ETIME so the
	 * clockevent core can re-arm with a larger delta rather than spin
	 * here forever with interrupts disabled.
	 */
	for (retries = 0; retries < MAX_DELTA_RETRIES; retries++) {
...
/*
 * Sets the state (shutdown) of a timer.  Disarm the H2 timeout by pushing it
 * effectively infinitely far into the future so no stale deltatimeout fires
 * after the device is detached.
 */
static int set_state_shutdown(struct clock_event_device *evt)
{
	__vmtimerop(settimeout, 0, ULLONG_MAX);
	return 0;
}
...
{
	*timer_val = __vmtimerop(gettime, 0, 0);
	return true;
}
