	...
	unsigned long addr, unsigned long sz)
{
	...
	if (WARN_ON_ONCE(!pmd_none(*pmd) && !pte_huge(*pte)))
		return NULL;
	if (WARN_ON_ONCE(!pmd_none(*(pmd+1)) && !pte_huge(*(pte+1))))
		return NULL;
	if (WARN_ON_ONCE(!pmd_none(*(pmd+2)) && !pte_huge(*(pte+2))))
		return NULL;
	if (WARN_ON_ONCE(!pmd_none(*(pmd+3)) && !pte_huge(*(pte+3))))
		return NULL;
