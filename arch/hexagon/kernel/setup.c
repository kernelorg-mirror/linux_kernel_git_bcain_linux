...
void __init setup_arch(char **cmdline_p)
{
	...
	p = NULL;
	for (magic=&external_buffer; magic < ((unsigned int *) &external_buffer)+32; magic++) {
		if (*magic == 0x54410009) {
			p=++magic;
			break;
		}
...
static int show_cpuinfo(struct seq_file *m, void *v)
{
	...
	if (cpu == 0) {
		...
		//  Macro might be nice
		seq_printf(m, "VM Build ID\t\t: 0x%08x\n", build_id);
		seq_printf(m, "VM Boot flags\t\t: 0x%08x\n", bootflags.raw);
		seq_printf(m, "   use_tcm\t\t: %d\n", bootflags.use_tcm);
		seq_printf(m, "VM STLB Info\t\t: 0x%08x\n", stlb_info.raw);
		seq_printf(m, "   enabled\t\t: %d\n", stlb_info.enabled);
		seq_printf(m, "   max_sets_log2\t: %d\n", stlb_info.max_sets_log2);
		seq_printf(m, "   max_ways\t\t: %d\n", stlb_info.max_ways);
		seq_printf(m, "   size\t\t\t: %d\n", stlb_info.size);
		seq_printf(m, "VM REV\t\t\t: 0x%08x\n", rev.raw);
		seq_printf(m, "   isa\t\t\t: %d\n", rev.isa);
		...
		seq_printf(m, "VM SYSCFG\t\t: 0x%08x\n", syscfg.raw);
		seq_printf(m, "   L2 Mem\t\t: %d\n", l2size);
		seq_printf(m, "   L2 Cache\t\t: %d\n", l2cache);
		seq_printf(m, "   TCM\t\t\t: %d\n", l2tcm);
