...
/*
 * Only H2 gives each virtual processor an identifier of its own; without it
 * the CPU number is the best the VM can be told apart by.
 */
static inline u32 hexagon_vpid(unsigned int cpu)
{
...
void send_ipi(const struct cpumask *cpumask, enum ipi_message_type msg)
{
	...
	for_each_cpu(cpu, cpumask) {
		...
		retval = __vmintop_post(CONFIG_HEXAGON_BASE_IPI_IRQ+cpu, per_cpu(vpid, cpu));

		if (retval != 0) {
			printk(KERN_ERR "interrupt %ld not configured?\n",
				CONFIG_HEXAGON_BASE_IPI_IRQ+cpu);
		}
	}
...
static void start_secondary(void)
{
	...
	per_cpu(vpid, cpu) = hexagon_vpid(cpu);
	...
	irq = CONFIG_HEXAGON_BASE_IPI_IRQ + cpu;
	__vmintop_globen(irq);
	if (request_irq(irq_create_mapping(hexagon_irq_domain, irq), handle_ipi,
...
{
	int i, irq = CONFIG_HEXAGON_BASE_IPI_IRQ;
	...
	if (max_cpus > 1) {
		__vmintop_globen(irq);
		if (request_irq(irq_create_mapping(hexagon_irq_domain, irq),
				handle_ipi, IRQF_TRIGGER_RISING,
				"ipi_handler", NULL))
			pr_err("Failed to request hwirq %d (ipi_handler)\n", irq);
	}

	per_cpu(vpid, smp_processor_id()) = hexagon_vpid(smp_processor_id());
}
