...
/*
	The IO mappings are based upon 4K pages regardless of what PAGE_SIZE is. So we cannot
	use the paging macros. - mikemac 3/14/2012

	Actually that's not totally true...  some of them can use larger page sizes; it's just that
	the smallest unit of granularity is 4k.
	
	This MSM_CHIP_DEVICE table is mapped very early on in init (see devicemaps_init).
	No reason why it can't just be done in board.c.  
	
	Seems to be a part of the platform mess.

	msm_shared_ram_phys is changed in board-8960.c to:

	MSM_SHARED_RAM_PHYS;

	Ugh...  feel dirty doing this.
*/
...
#define __phys_to_pfn(paddr)	((unsigned long)((paddr) >> 12))
#endif

static void __init create_single_pagemapping(unsigned long addr, unsigned long pfn)
{
	pmd_t *pmd;
	pte_t *pte;
	int index;

	pmd = (pmd_t *) (pgd_offset_k(addr));

	if (pmd_none(*pmd)) {
		int i;
		//  The only reason this works is because our board init is moved into initcalls, which happens after paging_init.
		pte = kmalloc(PTRS_PER_PGD * sizeof(pte_t), GFP_KERNEL);
		for (i = 0 ; i < PTRS_PER_PGD ; i++)
			pte_clear(&init_mm, addr, pte+i);
	
			set_pmd(pmd, __pmd(virt_to_phys(pte)));
		}

		if ((pmd_val(*pmd) & (~__HVM_PDE_PTMASK_4KB)) != 0) {
			printk(KERN_WARNING "BUG: not creating mapping for 0x%08lx"
				" at 0x%08lx in user region - already EXISTS as L1 direct mapping!\n",
				pfn << 12, addr);
			return;
	}
	pte = (pte_t *) phys_to_virt(pmd_val(*pmd));
	index = (addr >> 12) & (1024 - 1);
	if (pte_none(pte[index])) {
		set_pte(pte+index, __pte((pfn << 12) | (__HVM_PTE_R | __HVM_PTE_W | __HVM_PTE_X \
						| __HEXAGON_C_DEV << 6)));
		return;
	}
	printk(KERN_WARNING "BUG: not creating mapping for 0x%08lx"
		" at 0x%08lx in user region - L2 mapping already EXISTS!\n",
		pfn << 12, addr);
}

static void __init create_mapping(struct map_desc *md)
{
	unsigned long addr, pages, pfn;
	int i;

	addr = md->virtual & ~((1 << 12) - 1);
	pages = (md->length + ((1 << 12) - 1)) >> 12;
	pfn = md->pfn;

	printk("map 0x%08lx to 0x%08lx, len %ld\n",
		addr, pfn << 12, pages);

	for (i = 0 ; i < pages; i++) {
		create_single_pagemapping(addr, pfn);
		addr += (1 << 12);
		pfn += 1;
	}
}
