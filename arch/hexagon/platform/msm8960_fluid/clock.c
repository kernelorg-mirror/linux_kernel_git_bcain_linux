...
out:
	...
	printk("%s clk %s returning %d\n", __func__, clk->dbg_name, ret);
	return ret;
err_prepare_clock:
...
out:
	printk("%s clk %s returning %d\n", __func__, clk->dbg_name, 0);
	return ret;  //  howsabout we return ret
...
static enum handoff __init __handoff_clk(struct clk *clk)
{
	...
	if (ret == HANDOFF_ENABLED_CLK) {
		...
		if (ret == HANDOFF_ENABLED_CLK) {
			printk("%s was enabled by bootloader\n", clk->dbg_name);
		...
		printk("%s's parent (%s) was disabled after bootloader\n", clk->dbg_name, clk_get_parent(clk)->dbg_name);
		}
	}
	else {
		printk("%s was disabled after bootloader\n", clk->dbg_name);
	}
out:
	if (err) {
