			...
			struct device *dev)
{
	...
	//  Great the interface for this changed...  what config!?
	//rdev = regulator_register(rdesc, dev, &(pdata->init_data), vreg, NULL);
	//  Make sure CONFIG_REGMAP is disabled or we'll be paddling.
	reg_cfg.dev = dev;
	reg_cfg.init_data = &(pdata->init_data);
	reg_cfg.driver_data = vreg;
	reg_cfg.of_node = NULL;
	reg_cfg.regmap = NULL;
	rdev = regulator_register(rdesc, &reg_cfg);
	if (IS_ERR(rdev)) {
		rc = PTR_ERR(rdev);
