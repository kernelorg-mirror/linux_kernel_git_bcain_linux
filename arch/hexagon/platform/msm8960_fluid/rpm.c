...
static inline long h2_pic_get_status(int irq) {
	//  fixme:  constants bad
	//  also, reaching directly into the controller is bad
	struct irq_data *irq_data = irq_get_irq_data(irq);
	return __vmintop_status(irq_data->hwirq);
}

static inline long h2_is_irq_pending(int irq) {
	return h2_pic_get_status(irq) & 1;
}

static inline h2_clear_irq_pending(int irq) {
	struct irq_data *irq_data = irq_get_irq_data(irq);
	__vmintop_clear(irq_data->hwirq);
}

#define gic_is_irq_pending(x) h2_is_irq_pending(x)
#define gic_clear_irq_pending(x) h2_clear_irq_pending(x)
	...
	uint32_t *sel_masks, struct msm_rpm_iv_pair *req, int count)
{
	...
	if (msm_rpm_request) {
		printk("waiting for previous request?\n");
...
int __init msm_rpm_init(struct msm_rpm_platform_data *data)
{
	...
	printk("%s irq %d status 0x%08x\n", __func__, data->irq_ack-1, h2_pic_get_status(data->irq_ack-1));
	printk("%s irq %d status 0x%08x\n", __func__, data->irq_ack, h2_pic_get_status(data->irq_ack));
	...
	printk("%s irq %d status 0x%08x\n", __func__, data->irq_err, h2_pic_get_status(data->irq_err));
	...
	printk("%s irq %d status 0x%08x\n", __func__, data->irq_wakeup, h2_pic_get_status(data->irq_wakeup));
