...
#define GPIO_OFFSET(off)     ((off) - MSM_TLMM_BASE)
...
{
	if (on)
		set_gpio_bit(n, gpio_base + (int)dev->regs.out);
	else
		clr_gpio_bit(n, gpio_base + (int)dev->regs.out);
}
...
static int gpio_chip_direction_input(struct gpio_chip *chip, unsigned offset)
{
	...
	clr_gpio_bit(offset, gpio_base + (int)msm_gpio->regs.oe);
...
gpio_chip_direction_output(struct gpio_chip *chip, unsigned offset, int value)
{
	...
	set_gpio_bit(offset, gpio_base + (int)msm_gpio->regs.oe);
	spin_unlock_irqrestore(&msm_gpio->lock, irq_flags);
...
static int gpio_chip_get(struct gpio_chip *chip, unsigned offset)
{
	...
	spin_lock_irqsave(&msm_gpio->lock, irq_flags);
	ret = readl(gpio_base + (int)msm_gpio->regs.in) & bit(offset) ? 1 : 0;
	spin_unlock_irqrestore(&msm_gpio->lock, irq_flags);
...
static int msm_gpio_probe(struct platform_device *dev)
{
	...
	struct piranha_gpio_platform_data *pdata =
		(struct piranha_gpio_platform_data *)dev->dev.platform_data;
	int ret;
	...
	memcpy(&msm_gpio->regs,
	       &pdata->regs,
	       sizeof(struct piranha_gpio_regs));
