...
struct hexagon_ss_register registers[] = {
	...
	{ .name = "PLL_OPMODE",			.offset = 0x22c },
	{ .name = "PLL_STATE",			.offset = 0x230 },
	{ .name = "PLL_DROOP",			.offset = 0x234 },
	{ .name = "PLL_FRAC_VAL",		.offset = 0x238 },
...
ssize_t clock_dump_read(struct file *filp, char __user *ubuf,
	size_t cnt, loff_t *ppos)
{
...
ssize_t clock_dump_write(struct file *filp, const char __user *ubuf,
	size_t cnt, loff_t *ppos)
{
	return cnt;
}
...
static int __init v60_debug_init(void)
{
	...
	hexagon_pub_base = ioremap_nocache(0x08300000, PAGE_SIZE);
