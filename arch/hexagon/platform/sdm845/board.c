/*
 * linux/arch/hexagon/platform/sdm845/board.c
 *
...
static const char *sdm845_dt_compat[] __initconst = {
	"qcom,sdm845",
	NULL
};
...
static int __init sdm845_init(void)
{
	printk("%s\n", __func__);

	if (strcmp(mdesc->name,"sdm845")) {
		printk("omg not sdm845!!!\n");
		return 0;
	}
 ...
 */
arch_initcall(sdm845_init);


void setup_arch_platform_sdm845(void)
{
	printk("%s\n", __func__);
...
MACHINE_START(SDM845, "sdm845")
	.setup_arch_platform = setup_arch_platform_sdm845,
	.dt_compat = sdm845_dt_compat
MACHINE_END
