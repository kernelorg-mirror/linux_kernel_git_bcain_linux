...
long __vmnewmap(void *map, unsigned long type, unsigned long tlb_flush_flag);
long __vmcache(enum VM_CACHE_OPS op, unsigned long addr, unsigned long len);
unsigned long long __vmgettime(void);
long __vmsettime(unsigned long long time);
long __vmstart(void *pc, void *sp, int relprio);
void __vmstop(enum VM_STOP_STATUS);
long __vmwait(void);
void __vmyield(void);
long __vmvpid(void);
unsigned long __vmgetinfo(unsigned long type);

unsigned long __vmpmucfg(unsigned long operation, unsigned long r1, long r2,
			unsigned long r3);

long __vmhwconfig(unsigned long op, unsigned long r1, unsigned long r2,
		  unsigned long r3);
